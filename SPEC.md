...
Design (revised 2026-08-03): **light and dark palettes** in `css/theme.css` — cool neutral surfaces, violet accent, entries as bordered horizontal cards with hover lift. An inline `<head>` script resolves the theme before first paint (stored choice, else `prefers-color-scheme`) and stamps `data-theme` on `<html>`, so there's no flash and no duplicated media-query block; a switch in the site bar sets an explicit choice, and with none stored the page keeps following OS changes live. Full-bleed site bar (title + About), subtitle + stat figures, then a sticky toolbar carrying the search field *and* the category chips (horizontally scrollable on small screens); Section/Year/Tags collapse behind "More filters". Category badges carry inlined Lucide icons. All colors as custom properties on `:root`.
